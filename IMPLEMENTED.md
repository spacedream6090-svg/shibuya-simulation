...
| 第123 | **受領文書の格納(A7消化)+公開ミラーの穴1件封鎖+Codex接続準備(docsとopsのみ・コード不触)**: 監査5本(SHIBUYA_SIMULATION_*)+GPUサーバー引き継ぎメモを root直下→docs/plans/source/ へ移動(root=ミラー除外対象外のため未コミット留置だった件)。**★ops/ はミラー対象と再確認**→サーバー識別子(hostname/内部IP/SSHユーザー)を含む ops/setup-gpu-sv-002.md・ops/codex-review-pack.md を publish_public_mirror.ps1 の除外リストへ追加+**全履歴の識別子内容検査(トリップワイヤ)**を新設。ミラー最終同期=8/12・識別子コミット=8/16→**未公開のまま封鎖**(既公開履歴ハッシュ不変=fast-forward維持・VPN公開IPはコミット済みファイルに不存在をgrep確認)。Codex接続=ローカルed25519鍵生成+~/.ssh/config(gpu-sv-002)・VPN疎通OK・残る手動=公開鍵登録1ペースト+codex login | 2026-08-16 | docs/ops のみ |
