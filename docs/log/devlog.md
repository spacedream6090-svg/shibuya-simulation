...
### Entry 26 — 2026-07-20 — P2設計承認(決定4件)+Wave2起動
- **ユーザー決定**: ①C2会話の既定密度=**密~30/人日**(渋谷の密度感を最初から。較正で調整)
  ②S7方針キャッシュ=**実装はするが既定OFF運用**・本番で使うかは比較実験で決める(関門=呼数k非依存
  +ブラインドA/Bはそのまま) ③Gumbelロジット化=採用(温度は活動3分類固定) ④3D Phase 0(z列+
  O(1)地形サンプリング)=P1に同梱。
- **Wave2起動**: P5ペルソナ100万生成(Opus・決定論95%+LLM対象リスト出力・巨大データはgitignore・
  議員名簿は小ファイルでコミット可)/P2-S2ナラティブ補間(Opus・digest_line同型seam・既定OFF)。
  P1(SoA+hydrate+z列+P0で特定した純オーバヘッド除去+密度結合の有界化)はFable直営で着手。
- Wave1の全成果はコミット済み(P0=3b7039b・P6=b62ceac・P4=64c9013・リサーチ3本・設計書2冊・SUMO)。
