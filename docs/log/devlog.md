...
### Entry 27 — 2026-07-20 — Wave2前半完遂: P5プール100万+S2ナラティブ補間+P1エンジン4倍
- **P5**(検収・コミット済): 100万プール決定論生成(L1住民3.0万/L2従業者25.4万=台帳需要整合/
  L3定期3.7万/L4回転67.9万/L5役割1,020)。議員34=渋谷区議会実定数。実体736MBはgitignore・
  生成器+テスト14本+議員名簿+統計サマリをコミット。LLM上塗り対象リスト(llm_targets.json)出力済み。
- **P2-S2**(検収・コミット済): ナラティブ補間=有界リングバッファ(30件)→機械ダイジェストを
  全LLM推論に注入+夜内省の物語化。既定OFFバイト一致・呼数k非依存・決定論。テスト8本。
  フルスイート1019緑(32:53)。
- **P1直営(a+b)**: 挙動完全同一の3変更でエンジンを桁改善——(a)valence有界メモ化+networkx
  dispatch層バイパス=cが半減(300: 0.000420->0.000238) (b)timeline_forのdict線形in判定をid集合化=
  **N=3000で c 0.001660->0.000418(4.0倍)・超線形が線形圏内(3xNでc比1.13x)へ**。
  ev/agent-step全regime不変=挙動同一の傍証。0.0002目標が現実の射程に入った。
- 残るP1: config巻き上げ・score事前計算・SoA+z列+hydrate・大Nでの再計測。
  次スライス: S3会話3層(C2=30/人日)。

### Entry 28 — 2026-07-20 — P2-S3会話3層完成(記録に残る会話=30.9/人日)
- C2構造化会話: hearers_of近傍の知り合い対をstream c2_meet で抽選→Dialogue Act列を決定論遷移で
  生成(実文なし)→関係更新・FJ意見・語彙接触・driveゲージへ機械効果→conversationイベント1件記録。
  既定パラメータでn=200基準**30.9会話/人日**=ユーザー決定の密30に一致。C2→C1昇格はdrive経由。
  C3=すれ違い/挨拶の日次カウンタ(S2ダイジェストの材料)。
- R1: 新stream 1本・既定OFFでconversationイベント0件=ゴールデンバイト一致・LLM呼数不変・
  no-fingerprint。フルスイート**1034 passed**(34:46)。
- これでユーザー要望「重要な会話は毎回LLMでなくていいが記録に残る形に」が本稼働: 記録に残る
  認知・社会イベント=朝計画1+夜内省1+熟慮~6+C2会話~30+寄り道等(S4予定)≈**~40/人日**が
  ON時の姿(現実の「1日数十回の相互作用」レンジに到達)。
- 残: S1計画コンパイラ・S4確率的実行(Gumbel込)・S5退屈ドライブ・S6予算N比例+重畳・S7(既定OFF)・
  P1続行(config巻き上げ・SoA+z列+hydrate)・P3ローテーション。

### Entry 29 — 2026-07-20 — P2-S1+S4完成(計画フレームワーク+行動のゆらぎ)
- **S1**(1041緑): 型付き日課スキーマ(自由文intent先頭・mandatory/maintenance/discretionary・
  flex属性・同伴者・失敗時代替)+アンカー先置きコンパイラ+失敗の階段(決定論修復→再試行1回→
  前日流用)。mockにもスキーマ準拠応答を追加。day_plan_compiledイベント新設。
- **S4**(1052緑): 実証3層のゆらぎ——L1 motif(novelty 15%)/L2 ±30分ジッター(fixedアンカー不変)/
  L3 寄り道per-move実測0.153(目標10-20%帯)+中断/Gumbelロジット化(温度: mandatory0.3で74/80が
  いつもの行き先・discretionary1.5で80回中75種=温度効果明瞭)。専用stream5本・既定OFFバイト一致。
- 実行側の残ギャップ(S4報告): day_schedule→実行の精緻な配線はscheduler側=S6と同時に行う。
- ユーザー差し戻し「半ば決定論的になる」への回答が実装済みに: 計画は骨格・実行は実証値で較正
  されたゆらぎ+偶発遭遇(分布の非決定化)。「行間レイヤ」5スライス中4つ(S1-S4)が完成。

### Entry 30 — 2026-07-20 — P2-S5退屈ドライブ完成(行間レイヤ5/5)
- 退屈ゲージ(物理量のみ: 同一ノード長居で蓄積・移動で減衰・新ノードで解放)→閾値+cooldownで
  内発的探索をLLMなしで発火(近傍の未訪問/低頻度POI・S4 Gumbel再利用・stream boredom 1本)。
  未訪問優先が既存novel_place→顕著性鋭敏化に自然接続=探索先で何かあればC1熟慮へ。
  既定0.57回/人日(保守)〜3.6回/人日(強め)を較正可能。テスト13本・フルスイート**1065緑**。
