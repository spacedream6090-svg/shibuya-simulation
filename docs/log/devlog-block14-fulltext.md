# 開発ログ 全文アーカイブ — Block #14(Entries 71〜80・2026-07-31 〜 08-01)

> [devlog.md](./devlog.md) のライブエントリ 71〜80 の全文退避(圧縮版は [devlog-compressed.md](./devlog-compressed.md) Block #14)。
...
---
### Entry 79 — 2026-08-01 — 天候W1検収: 東京8月930日凍結+生成器較正(猛暑連長KS p=0.97・32緑)
Opus実装(B)をFable検収。新規7ファイル(fetch/fit/凍結2JSON/SOURCE.md/設計書/テスト32本)・src/confゼロタッチ確認。
- **リサーチの核心**(出典付き): WGEN系(Richardson 1981=降水マルコフ+気温AR(1))は**持続を過小評価**し
  (J.Climate 2001)、月次/年々分散も過小(Chen+2010)→**年効果(低周波)項が連続猛暑再現に必須**を文献+
  実証の両方で確認(年効果を外すと5連以上確率が崩落)。気象庁ライセンス=公共データ利用規約1.0
  (CC BY 4.0互換・出典明示で同梱可)を確認済み=リポ同梱OK。WBGT=小野ら2014環境省式。
- **較正**(1996-2025凍結・較正窓は観測点移転後2015-2025): P(≥35℃)実測23.5%→生成22.1%(現行合成14.4%)・
  猛暑連長平均2.22→2.05(現行1.16)・**KS(連長)D=0.081 p=0.97**(現行p=2e-5)・2025年の10日連続猛暑も
  生成器で到達可能。P(wet|wet)=0.505・年効果SD1.35℃。決定論バイト同一。
- **重要な発見**: P(≥35℃)は1996-2014の7.6%→2015-25の23.5%と3倍(温暖化+観測点移転が混在=較正窓の根拠)。
- 正直な限界: 地点=北の丸公園(渋谷から5.8km・都市キャノピー補正なし)・生lag-1自己相関0.39vs実測0.52
  (WGEN構造の限界・連長には非影響)・P(≥36℃)1.8倍過大(歪度未対応)・8月のみ較正。
- 統合設計(W2引き継ぎ): mode generated=**全系列一括決定論生成でcheckpoint無改修**・weather_line文言不変・
  paramsはenvpack経由・summaryにweather_params_sha256(=入力来歴S0の最初の実例)。検収=32緑(src非依存を
  import実査して並行A作業中でも安全に実行)。

---
### Entry 80 — 2026-08-01 — 第79バッチ検収: 定数の毎分レート化=Δt不変化(2231緑)+Block #14圧縮
Opus実装(A)をFable検収。新規2(timeconv.py=中央ヘルパ+分類テーブル・test_timeconv.py=49テスト)+16変更。
- **リサーチ**(出典付き): per-step確率は線形スケール不可=p(Δt)=1−(1−p₀)^(Δt/10)(ハザード保存)・
  Δt感度分析はABM検証の必須段(ACM TOMACS ほか)。
- **第4分類の発見**: 指示の3分類(rate線形/prob べき/invariant)に加え**steps類=逆比例**(滞在長・cooldown・TTL)
  を実査で発見。「カウントだから不変」で放置するとΔt=5で滞在・睡眠が全部半分になる罠を回避。
  分類=rate13/prob26/steps31/invariant60(計130 conf キー+コード直書き=Clock単一源化・リテラル6箇所)。
- **設計の要**: 変換はload_configの1箇所(下流数百箇所はΔtを知らない)・**Δt=10は変換式を通さない構造分岐**
  (同一オブジェクト返却=golden担保)・dt_minは1440の約数強制・n_stepsは自動換算しない(黙って10倍の危険)・
  網羅性は棚卸しgrep→TABLE全載をCIテスト化(宣言忘れ構造的防止=registry/第72と同型)。
- 実測: 既定=L1バイト一致・呼数119=119。Δt=5/1スモーク完走(行動量は2倍以内・全イベント増はmove_segment等の
  観測行が支配)・Δt=5でresume一致。フルゲートxdist **2231緑**(354s=2150+49+天候32)。
- 限界: Δt≠10は別世界(乱数列変化=許容設計)・Δt=1はログI/O10倍・SUMO/synth_crowdのオフライン工具に
  STEP_SECONDS残存(範囲外と正直記録)。→天候W2(weather.py統合)起動。本エントリで10/10=Block #14圧縮。