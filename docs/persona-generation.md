...
- **多軸構造(Wave2拡張)**: `ontology.axes.<軸名>` で文化圏と**直交する追加軸**を任意本数定義できる
  (軸ごとに独立ハッシュ=軸間の割当は無相関を実測検証済み)。第1弾=**情報行動軸**(SNS一次/使い分け/
  公式・放送の3群・`agent.age` による年代条件付き構成比=公的統計の年代勾配を再現)。
  **防災訓練経験**は文化圏群への付加行(`drill_line`・`drill.enabled` で入切)。軸候補の採点と
  選定根拠は [research/ontology-extensions.md](./research/ontology-extensions.md)(世代論・Hofstede
  個体適用は根拠薄で不採用の negative finding も記録)。
