...
| 4b | **★正直な相互作用(fire ON + tiers ON でのみ起きる)**: DPH-B が計画/内省を繰り越すと、その個体は `plan_step`/`reflect_step` が翌 step へ立つので、`fire.note_plan_due` → `_social_via` が **繰り越した step 数だけ `cog_event{via:"plan"/"reflect"}` を出し、そのたびに周期発火を先送りする**(`fire.py:412-556`)。fire OFF の現状では 1 行も走らないので無風だが、**3行を解凍するときはここを最初に見ること**。対処案は 2 つ:(a) `max_defer_steps` を小さくして繰り越し自体を短くする (b) `note_plan_due` を「初回予約 step だけ」に絞る(= `plan_due_step` を見る 1 行)。どちらも fire を開ける判断とセットで決める | ⬜ **未対処**(fire OFF のため現状は無影響) |
