...
> **改訂注(8/16外部監査トリアージ後)**: このT10表はDPH-B(tiers)以前の呼数前提=保守側に古い。現行confの真の呼数レジームとcap再導出表は [external-audit-triage.md](external-audit-triage.md) §2 が正。8/22開始の結論は不変(余裕はむしろ増える)。
...
> **β追補(8/16・外部監査2本のトリアージ)**: β6=mock fail-fast+凍結conf・β7=checkpoint stream書き・β8=呼数cap再導出(DP-U4)・β9=HOME_AWAKE最小(要判断)。採否と根拠は [external-audit-triage.md](external-audit-triage.md) §3-§5(判断A1〜A7)。
