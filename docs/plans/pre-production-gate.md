...
| # | 項目 | 判定値・線 | 確認方法(1行) | 担当 | 状態 |
|---|---|---|---|---|---|
| B1 | **mock fail-fast ガード** | `run.n_agents >= 10,000` ∧ `model.backend == "mock"` ∧ `run.allow_mock_production == false` → **起動時 RuntimeError**。逃し弁は `run.allow_mock_production=true`(スケールスモーク用)。**打ち間違い1つで25万体 mock ラン**を防ぐ保険(監査 E0-4 / F5) | `python scripts/run.py --profile conf/finals_observe.yaml run.n_agents=10000 pool.present_cap=10000 run.n_steps=1` が即 raise すること(present_cap の対指定は B4 の運用線。ガードは present_cap を見ないので判定は変わらない) | F | ✅ 第121(`tests/test_launch_guard.py`) |
