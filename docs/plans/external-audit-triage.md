...
| A7 | 監査2本の扱い: リポ直下に未コミットのまま置かれている。**root直下は公開ミラーの除外対象外**=コミットすると次回同期で公開される。**推奨=docs/plans/source/へ移してコミット**(ミラー除外域・受領文書の置き場) | 移動+コミット | ✅ 2026-08-16消化(第123・5本+引き継ぎメモをsource/へ) |
...
→ **✅ 2026-08-16消化(第123)**: 5本+GPUサーバー引き継ぎメモを docs/plans/source/ へ移動してコミット。同便で公開ミラーの穴を1件塞いだ: **ops/ はミラー対象**のため、サーバー識別子(hostname/内部IP/SSHユーザー)を含む ops/setup-gpu-sv-002.md・ops/codex-review-pack.md を除外リストへ追加+全履歴の識別子内容検査を追加。両ファイルとも前回同期(8/12)以後のコミットのみ=**未公開のまま防止**(fast-forward維持)。
