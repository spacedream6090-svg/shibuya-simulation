...
| P4-3 | **Tordeux型 V(s)=前方間隔ベースの希望速度**(P4-1 の分岐条件成立=長距離項では ρ=3.0 の+2.9倍が消えず単調性2違反が残るため必要)+**壁斥力の較正**(P4-1 新規発見=WALL_A/B の過剰クリアランス片側0.15m→w≤0.9m 通過不能・J/w 水準 1/2.2〜2.8 の主因候補) | 中 | reference/(較正)→src/society/physics.py(昇格・既定OFF) | P4-1 実測・リサーチ §4(JuPedSim 製品モデル=Tordeux/Chraibi/Seyfried 2015) |
