...
| P4-2 | 較正結果の本体反映(**P4-1 実測で確定**): `physics.sfm.far_field{enabled:false, a2:0.119, b2:1.890, cutoff_factor:2.5, taper_m:1.0}` を既定OFFで昇格(カットオフ規則は A2 とセット)・**λは0.5据え置き**(λ先行実験が文献値仮説を棄却=下げると非単調性2倍悪化)+受入テスト(**局所測定密度**の包絡線内率+1step変位上限。単調性はP4-3側の受入) | 中 | src/society/physics.py・conf・tests | P4-1 実測(calib_results.json) |
| P4-3 | ~~Tordeux型 V(s)+壁斥力較正~~ → **完了(2026-08-05)・ただし起動条件は覆った**: ①測定の物差し是正(RiMEA 2×2m 区画は幅3m通路で外側各0.5mを落とす→実測側と同じ「流れ方向2m×通路全幅」に統一)で**現行既定のままでも C 0.60→1.00・A 0.50→0.80** ②★**ρ_global≥2.0 の FD 点は壁貫通で個体が消えた系を測っていた**(ρ=3.0 で 90/180 が通路外へ脱出・対人斥力 clip 後 1365 m/s² が壁1本を上回り v_max クリップが向きを保存して4サブステップで貫通)=P4-1 の「+2.9倍」はこの汚染込み ③是正後: far_field ON で **A=1.00✓/B=0違反✓/C=1.00✓**・**V(s) の限界効用はほぼゼロ**(むしろ微悪化)→実装・conf 化は保全しつつ既定OFF据え置き ④壁較正: B_w≤0.06 で w=0.9m 単独通過达成・**D(J/w 水準)は未達のまま**(壁主因仮説は部分否定・残候補=接触項不在/τ/v0分布/定常部の取り方) | 完 | reference/calibrate_p43.py・src/society/physics.py(_CalibratedCrowd・既定OFF恒等5証明) | calib_p43_results.json |
