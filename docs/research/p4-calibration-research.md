...
## 追記(2026-08-05・P4-2/3 実測後)

本文 §6.2 が P4-3(間隔ベース V(s))の起動条件とした「長距離項では単調性が通らない」は、**実測で覆った**:

1. **測定の物差し是正**: RiMEA Test 4 の 2×2m 区画を幅 3m 通路にそのまま置くと外側各 0.5m が測定から落ちる。実測側(juelich.py)と同じ「流れ方向 2m × 通路全幅」に揃えると、**現行 src 既定のままでも C 包絡線内率 0.60→1.00・A ±20%帯 0.50→0.80**。
2. **高密度 FD 点の汚染**: ρ_global ≥ 2.0 の点は**壁貫通で個体が通路外へ脱出した系**を測っていた(ρ=3.0 で 90/180 脱出)。対人斥力(clip 後 1365 m/s²)が壁 1 本を上回り、v_max クリップが向きを保存するため 4 サブステップで貫通する。WALL_B 縮小では直らない(接触項 or v_max クリップの再設計が要る=別課題)。**P4-1 の「ρ=3.0 で +2.9 倍」はこの汚染を含む**。
3. 是正後、far_field ON だけで A=1.00 / B=0 違反 / C=1.00。**V(s) の限界効用はほぼゼロ**(最大相対誤差 0.113→0.131 と微悪化)。V(s) は実装・conf 化済みだが既定 OFF 据え置きが実測根拠。
4. D(ボトルネック J/w 水準)は全構成で未達のまま。壁主因仮説(P4-1)は部分否定され、残候補は接触項の不在 / τ=0.5s / v0 分布(1.19 vs 実測 1.41)/ 定常部の取り方。

数値の正典: `reference/physics_bench/out/calib_p43_results.json`・README §7〜§12。
