...
| レーン | 内容 | 状態 |
|---|---|---|
| ~~IF-E2 案B~~ | → **完了**(2026-08-07・漏れ 96%→0.00%・検査①全部門 PASS)。→ IMPLEMENTED。残=UNCOVERED 4種(rule_bonus/crime/chance_event/b2b_trade)・倒産機構(B-2)・org 資金繰りの行動反映(いずれも将来拡張=docstring 宣言済み) | **完了** |
...
- **IF-E2 残**: ①UNCOVERED_KINDS 4種(rule_bonus/crime/chance_event が変更範囲外ファイルで残高を動かす・b2b_trade は帳簿のみ=点火するランでは不変量の外)②屋台の内税/床クリップギャップ(RoW が埋める=改名して隠さない)③**日カウンタの同型 checkpoint 未保存ギャップ残存**(`_freedom_day`/`_vc_review_day` ほか=`_work_day` と同型・mid-day resume で二重記録の恐れ)
