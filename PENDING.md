...
| **IF-U1** | **LLM⇄世界インターフェース増強**([監査](docs/research/llm-world-interface-audit.md)): 外部相談4IF案の実診断=①行為API②観測パケットはほぼ完全実装・薄いのは情報オブジェクト一般化/痕跡/world_diff。実装候補 IF-1(行為イベント llm_call_id)〜IF-5(会計接続)+穴3件(contingency 未消費・_apply の plan/recall/reflect 無音落ち) | **新規**(2026-08-05)。ユーザー指示で実装保留中。推奨=IF-1+穴3件は判断不要枠で小さく・IF-2〜IF-4 は本選前に入れるか要判断(創発の観察に直結) |
