...
| **IF-U1** | **LLM⇄世界インターフェース増強**([監査](docs/research/llm-world-interface-audit.md)・[計画](docs/plans/if-sv-p4-plan.md)): ~~実装保留~~ → **実装承認済み**(2026-08-05「IF-U1の実装を始めて」)。**IF-A 完了**(=IF-1+穴2件)・**IF-B 完了**(拒否通知3水準+第87脱出条件(1)実バグ修正)。残=IF-C(噂オブジェクト+stifler)/IF-D(痕跡=集約+蒸発)/IF-E(会計=検査先行)を順次実装中。**注意**: 第87 の engaged 実測値(7.38エピソード/滞在30.4%)は脱出条件(1)不発火バグ込みの測定=8/15-16 の θ_in 再較正で再測(DP-U4 と同時) | IF-A/B **完了**・IF-C〜E 実装中 |
