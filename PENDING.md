...
| ~~物理ゾーン痩身の方式~~ | **ユーザー決定(2026-08-16)=A+B+C全部**(認知二層指示→文献リサーチ docs/research/crowd-attention-physics.md で裏付け: 視覚的近傍が実証最良・遠方はensemble知覚=全ペアは認知モデルとしても過剰)。A=同値セル法+観測層ベクトル化(軌跡ビット一致・実装中)→B=認知的近傍(cap+セクター遮蔽・基本図ベンチ再較正)→C=far項の密度場化。B/CはAの着地後に1レーンで | **決定済み・A実装中** |
