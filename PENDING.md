...
| **IF-U1** | **LLM⇄世界インターフェース増強**([監査](docs/research/llm-world-interface-audit.md)・[計画](docs/plans/if-sv-p4-plan.md)): ~~実装保留~~ → **実装承認済み**(2026-08-05「IF-U1の実装を始めて」)。**IF-A 完了**(=IF-1+穴2件・(llm_call_id,role) 対+contingency 消費+misrouted 分岐)。残=IF-B(拒否通知・Ovsiankina 根拠)/IF-C(噂オブジェクト+stifler)/IF-D(痕跡=集約+蒸発)/IF-E(会計=検査先行)を順次実装中 | IF-A **完了**・IF-B〜E 実装中 |
