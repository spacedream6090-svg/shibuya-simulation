...
| **★v2プール2系統分岐** | ローカル8/15生成(L2 138,759=出勤率0.62設計どおり)とサーバー8/16生成(L2 157,715=0.703相当・差=ちょうど18,956人のL4→L2シフト)が併存。**本選はサーバーのプールで走るため正典未決**。切替前に現HEADから決定論再生成→どちらが再現されるか確定→生成条件差の特定→conf表とtier_quota引き直し(conf/finals_observe.yaml:93付近に未解決印) | **新規(第133)・v2切替前必須** |
| **★v2に配達員が不在** | v1の配達員1,124/バンドマン917/写真家871がv2に無い(両ビルド共通)。`delivery.courier_occupation`が配達員名指しのため**v2切替で宅配の実配車とgig収入が全停止**。3案=(a)v2生成に職業追加(再生成) (b)confを宅配ドライバー(830人)へ差し替え (c)放置(劣化承知)。confに記録済み | **新規(第133)・v2切替前必須** |
| **★物理ゾーン痩身の方式** | prof10k6で物理=実行時間74%(ペア計算)が確定。25万体の成立性に直結。mock10k144完走値(RSS+Nスケール定数)と合わせて選択肢(近傍cap/セル法/サブステップ間引き/ゾーン人数上限)を提示予定 | **新規(第133)・mock完走後に提案** |
