...
| **IF-U1** | **LLM⇄世界インターフェース増強**([監査](docs/research/llm-world-interface-audit.md)・[計画](docs/plans/if-sv-p4-plan.md)): ~~実装保留~~ → **実装承認済み**(2026-08-05「IF-U1の実装を始めて」)。**IF-A 完了**(=IF-1+穴2件)・**IF-B 完了**(拒否通知3水準+第87脱出条件(1)実バグ修正)。**IF-A〜E 全完了**(2026-08-05・E は検査フェーズまで=接続は IF-E2 判断待ち)。IF-D 補足: trouble 階層の源(crime/nuisance)は diversity 層既定 OFF のため diversity OFF ランでは空振り・「痕跡が後続行動を変えたか」の DiD 解析は別バッチ。**注意**: 第87 の engaged 実測値(7.38エピソード/滞在30.4%)は脱出条件(1)不発火バグ込みの測定=8/15-16 の θ_in 再較正で再測(DP-U4 と同時) | IF-A〜E **完了** |
| **IF-E2** | **経済の会計主体化の採否**([実測](scripts/analyze_accounting.py) docstring に数値): ゼロ和の漏れ 71.8〜96.0%=「財布から出た金の受け手が世界に無い」。案A=観測のみ(保存改善0円・工数小)/案B=org を会計主体化(残高新設+支払不能規則=checkpoint/golden 波及・設計判断要)/案C=rest-of-world 部門と宣言(シム改変ゼロ・分類器の写像変更のみ)。付随発見=serve の 220/222 スタッフ不在で org 特定率が極端に低い・interest_paid が貨幣創出(Bank.capital 非減)・org/Bank 残高を L2 サイドカーへ出せば検査①が全部門で成立 | **新規**(2026-08-05)。推奨=本選前は**案C**(改変ゼロで報告の正直さを確保)+本選後に案B を検討 |
